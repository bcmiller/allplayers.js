(function($) {
  ...
  $.fn.treeselect = function(params) {
    ...
    params = $.extend({
      margin: '18px',         /** The margin between levels. */
    ...
    TreeNode.prototype.build_list = function() {
      ...
      if (this.id) {
        ...
        var oddEven = this.odd ? 'odd' : 'even';
        if (oddEven == 'odd') {
          var background = params.basecolor - (this.level * params.gradient);
          background = (background).toString(16);
          background = (background + background + background);
          list.css('backgroundColor', '#' + background);
        }
        list.addClass(oddEven);
      }
      return list;
    };
    ...
    TreeNode.prototype.build_children = function() {
      ...
      if (this.children.length > 0) {
        ...
        if (this.level > 0) {
          this.childlist.css('marginLeft', params.margin);
          this.childlist.css('marginRight', params.margin);
        }

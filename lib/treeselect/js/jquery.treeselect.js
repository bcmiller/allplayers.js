(function($) {
  ...
  $.fn.treeselect = function(params) {
    ...
    function expandClick(target, id, collapsed) {
      if (collapsed) {
        ...
        if ((list.length == 0) && params.load) {
          ...
          parent.append('<ul><li class="treebusy"></li></ul>');
          parent.find('ul').css('marginLeft', left).show('fast');
          params.load(id, function(tree) {
            parent.find('ul').empty().treeselect($.extend(params, {tree:tree}));
          });
        }
        else {
          $('ul[ref="' + id + '"]').show('fast');
    ...
    function render(tree, node) {
      ...
      if (node) {
        output += (childOut || node.has_children) ? '<span class="collapsed"></span>' : '';
    ...
    function makeTree(children, level, id, tree) {
      ...
      children.each(function() {
        ...
        input.attr({'ref': itemId, 'value': itemId});
        ...
        input.bind('click', {id: itemId}, function(event) {
          ...
          if (checked) {
            expandClick(event.target, event.data.id, true);
          }
        ...
        span.bind('click', {id: itemId}, function(event) {
          ...
          expandClick(event.target, event.data.id, collapsed);
        });
